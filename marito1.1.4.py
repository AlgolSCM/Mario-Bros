...
class player(object):
    def __init__(self,x,y,imagen):#self = mario
        self.x = x
        self.y = y
        self.velx = 18*k*k2
        self.vely = 13*k
...
##            self.walkCount +=1
         ...
         self.walkCount +=1
class cap(object):
    def __init__(self,a,b,imagen): # self = gorro
        self.x=a
        self.y=b
        self.coun=0.05
        self.velx=10
...
class enemy(object):
    def __init__(self,x,y,fin,width,height,vel,move,vidas,imagen):#self = enemigo
        self.x = (x*-1+20)*32
        self.y = (y*-1+15)*32
...
man = player(576, 416,'marito.jpg')
gorro = cap(200,410,'gorro1.png')
...
##            man.isJump = False
    ...
    while i<len(Ldeenemigos):
        ...
        if Ldeenemigos[i].vidas==0:Ldeenemigos.pop(i)
        i+=1
